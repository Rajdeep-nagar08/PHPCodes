...
//echo $aid;
 $con=@mysqli_connect('localhost','root','Raja@nagar08','airline') or die(mysql_error());
    	...
    	if($numrows > 0){
			...
			while($row=mysqli_fetch_assoc($result)){ ?>
				...
				<?php
					...
					$unseats = mysqli_num_rows($result);
					$availSeats = $tot_seats-$unseats;
				?>
	...
	if(!empty($_POST['flight_id']) && !empty($_POST['dptime']) && !empty($_POST['Booking_Method']))
	{
		...
		$payment = $_POST['Booking_Method'];
  		...
  		$con=@mysqli_connect('localhost','root','Raja@nagar08','airline') or die(mysql_error());
					...
					$booked_seats = "SELECT * FROM Records where Flight_ID='$fid' and Dep_Time='$var'";
					$res = mysqli_query($con,$booked_seats);
					$numseats = mysqli_num_rows($result);
		...
		$rows = mysqli_num_rows(mysqli_query($con,"SELECT* FROM records"));

		$rows++;

$sql="INSERT INTO records (Book_ID,Flight_ID,Dep_Time,Book_Time,User_Name,Payment_Type) values('$rows','$flight_id','$dptime','$today','$user','$payment')";
